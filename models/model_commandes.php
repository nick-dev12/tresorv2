...
function get_commande_produits($commande_id) {
    ...
    try {
        ...
        $join_variante = $has_var
            ? "LEFT JOIN produits_variantes pv ON cp.variante_id = pv.id"
            : "";
        $img = $has_var
            ? "COALESCE(pv.image, p.image_principale, '') as image_afficher"
            : "COALESCE(p.image_principale, '') as image_afficher";
        $var_nom = $has_var
        ...
        $stmt = $db->prepare("
            ...
            ORDER BY COALESCE(c.nom, '') ASC, COALESCE(p.nom, cp.nom_produit, '') ASC
        ");
...
function get_commandes_by_categorie($user_id, $categorie_id = null) {
    ...
    try {
        ...
        $join_pv = $has_var ? "LEFT JOIN produits_variantes pv ON cp.variante_id = pv.id" : "";
        $sql = "
            SELECT $cols
            FROM commandes cmd
        ...
        $sql .= " ORDER BY COALESCE(c.nom, '') ASC, cmd.date_commande DESC, COALESCE(p.nom, cp.nom_produit, '') ASC";
        
        $stmt = $db->prepare($sql);
        $stmt->execute($params);
        $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
        
        // Grouper par catégorie (lignes sans produit lié : clé dédiée)
        $grouped = [];
        foreach ($result as $row) {
            $cat_id = $row['categorie_id'];
            $cat_key = ($cat_id !== null && $cat_id !== '') ? (string) $cat_id : '_sans_categorie_';
            if (!isset($grouped[$cat_key])) {
                $grouped[$cat_key] = [
                    'categorie_id' => $cat_id,
                    'categorie_nom' => $row['categorie_nom'] ?? ($cat_key === '_sans_categorie_' ? 'Autres' : ''),
                    'produits' => []
                ];
            }
            $grouped[$cat_key]['produits'][] = $row;
        }
...
function get_produits_commandes_by_user($user_id, $statut_commande = null) {
    ...
    try {
        $has_nom_produit = _commande_produits_has_nom_produit();
        $nom_sel = $has_nom_produit
            ? "COALESCE(NULLIF(TRIM(cp.nom_produit), ''), p.nom) as nom"
            : "p.nom as nom";
        $order_nom = $has_nom_produit
            ? "COALESCE(p.nom, cp.nom_produit, '')"
            : "COALESCE(p.nom, '')";
        ...
        $sql .= " ORDER BY cmd.date_commande DESC, $order_nom ASC";
