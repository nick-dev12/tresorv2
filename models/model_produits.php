...
/**
 * IDs de tous les produits d'une catégorie (tous statuts)
 * @param int $categorie_id
 * @return int[]
 */
function get_all_produit_ids_by_categorie($categorie_id)
{
    global $db;
    try {
        $stmt = $db->prepare("SELECT id FROM produits WHERE categorie_id = :cid");
        $stmt->execute(['cid' => (int) $categorie_id]);
...
/**
 * Supprime les fichiers upload d'un produit (image principale + galerie)
 * @param array $produit Ligne produits (get_produit_by_id)
 */
function delete_produit_fichiers_upload($produit)
{
    if (!is_array($produit) || empty($produit)) {
        return;
    }
    $base = dirname(__DIR__) . '/upload/';
    if (!empty($produit['image_principale'])) {
        $path = $base . $produit['image_principale'];
        if (is_file($path)) {
            @unlink($path);
        }
    }
    ...
    foreach (array_filter(array_map('trim', explode(',', (string) $raw))) as $fn) {
        $path = $base . $fn;
        if (is_file($path)) {
            @unlink($path);
        }
...
/**
 * Supprime un produit (détache les lignes de commande / devis pour l'historique)
 * @param int $id L'ID du produit
 * @return bool True en cas de succès, False sinon
 */
function delete_produit($id)
{
    ...
    try {
        // ALTER éventuel : hors transaction (DDL = commit implicite MySQL)
        ...
        try {
            $st = $db->prepare("
                UPDATE commande_produits cp
                INNER JOIN produits p ON p.id = cp.produit_id
                SET cp.nom_produit = COALESCE(NULLIF(TRIM(cp.nom_produit), ''), p.nom)
                WHERE cp.produit_id = :id
            ");
            $st->execute(['id' => $id]);
        } catch (PDOException $e) {
            // Colonne nom_produit absente ou autre : poursuivre
        }

        $rsDevis = $db->query("SHOW TABLES LIKE 'devis_produits'");
        $hasDevisProduits = $rsDevis && $rsDevis->fetch();
        if ($hasDevisProduits) {
            try {
                $st = $db->prepare("
                    UPDATE devis_produits dp
                    INNER JOIN produits p ON p.id = dp.produit_id
                    SET dp.nom_produit = COALESCE(NULLIF(TRIM(dp.nom_produit), ''), p.nom)
                    WHERE dp.produit_id = :id
                ");
                $st->execute(['id' => $id]);
            } catch (PDOException $e) {
            }
        ...
        // Détacher les lignes (historique conservé via nom_produit / montants)
        try {
            $st = $db->prepare('UPDATE commande_produits SET produit_id = NULL WHERE produit_id = :id');
            $st->execute(['id' => $id]);
        } catch (PDOException $e) {
            // FK ou colonne : ensure_fk devrait avoir corrigé
        }

        if ($hasDevisProduits) {
            try {
                $st = $db->prepare('UPDATE devis_produits SET produit_id = NULL WHERE produit_id = :id');
                $st->execute(['id' => $id]);
            } catch (PDOException $e) {
            }
        }
