...
$facture_og_desc = 'Facture Trésor Africain - Demande personnalisée - Montant : ' . $montant_aff;
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .facture-banner-top { height: 60px; background: linear-gradient(135deg, rgba(194,102,56,0.25), rgba(145,138,68,0.2)); }
        ...
        .facture-logo { width: 100px; height: 100px; border: 2px solid #c26638; border-radius: 50%; overflow: hidden; flex-shrink: 0; }
        ...
        .facture-meta .solde { font-size: 16px; color: #c26638; margin-top: 8px; }
        ...
        .facture-table th { background: #c26638; color: #fff; font-size: 12px; font-weight: 700; padding: 14px 20px; text-align: left; }
        ...
        .facture-summary .total { font-weight: 700; font-size: 16px; padding-top: 12px; border-top: 2px solid #c26638; margin-top: 8px; }
        .facture-summary .solde-row { background: rgba(194,102,56,0.12); padding: 12px 16px; margin-top: 12px; border-radius: 6px; font-weight: 700; font-size: 16px; }
        .facture-banner-bottom { height: 40px; background: linear-gradient(135deg, rgba(194,102,56,0.3), rgba(145,138,68,0.2)); }
        .facture-actions { display: flex; flex-wrap: wrap; justify-content: center; gap: 10px; margin: 20px 0; }
        .facture-actions a { display: inline-flex; align-items: center; gap: 8px; padding: 10px 20px; background: #c26638; color: #fff; text-decoration: none; border-radius: 8px; font-weight: 600; font-size: 14px; }
        ...
        <div class="facture-header">
            <div class="facture-entreprise">
                <div class="facture-logo">
                    <img src="/image/logo.jpeg" alt="<?php echo htmlspecialchars($entreprise_nom ?? 'Trésor Africain'); ?>" onerror="this.style.background='#f8f7f2'">
                </div>
                <div class="facture-entreprise-info">
                    ...
                    <?php endif; ?>
                    <?php
                    $tels_cp = array_filter([trim((string) ($entreprise_tel1 ?? '')), trim((string) ($entreprise_tel2 ?? ''))]);
                    if (!empty($tels_cp)):
                        ?>
                    <p><i class="fas fa-phone" style="font-size:11px;"></i> <?php echo htmlspecialchars(implode(' / ', $tels_cp)); ?></p>
                    <?php endif; ?>
                    <?php if (trim((string) ($entreprise_site ?? '')) !== ''): ?>
                    <p><i class="fas fa-globe" style="font-size:11px;"></i> <a href="<?php echo htmlspecialchars($entreprise_site); ?>" target="_blank" rel="noopener noreferrer"><?php echo htmlspecialchars($entreprise_site); ?></a></p>
                    <?php endif; ?>
