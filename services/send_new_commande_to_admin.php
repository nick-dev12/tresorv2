...
function send_new_commande_to_admin($numero_commande, $montant_total, $nombre_articles, $telephone_livraison = '', $adresse_livraison = '', $produits = []) {
    ...
    if (!empty($admin_emails) && function_exists('mail_send')) {
        ...
        $body_html .= '<h2 style="color: #c26638;">Nouvelle commande reçue</h2>';
        ...
        $body_html .= '<p style="margin-top: 25px;"><a href="' . htmlspecialchars($link) . '" style="display: inline-block; padding: 12px 24px; background: #c26638; color: #ffffff; text-decoration: none; border-radius: 8px; font-weight: 600;">Voir les commandes</a></p>';
