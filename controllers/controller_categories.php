...
function process_delete_categorie($categorie_id) {
    require_once __DIR__ . '/controller_produits.php';
    ...
    $ids = get_all_produit_ids_by_categorie($categorie_id);
    foreach ($ids as $pid) {
        $res = process_delete_produit((int) $pid);
        if (!$res['success']) {
            return [
                'success' => false,
                'message' => $res['message'] . ' (lors de la suppression des produits de la catégorie).'
            ];
        }
    }
    ...
    if (delete_categorie($categorie_id)) {
        $n = count($ids);
        $msg = $n > 0
            ? ('Catégorie supprimée avec succès (' . $n . ' produit(s) supprimé(s)).')
